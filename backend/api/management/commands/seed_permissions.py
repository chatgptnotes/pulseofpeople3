...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        role_permissions_map = {
            ...
            'zone_admin': [
                # Similar to manager - zone level access
                # User Management
                'view_users', 'create_users', 'edit_users',
                # Data Access
                'view_dashboard', 'view_analytics', 'view_reports', 'export_data',
            ...
            'district_admin': [
                # Similar to manager - district level access
                # User Management
                'view_users', 'create_users', 'edit_users',
                # Data Access
                'view_dashboard', 'view_analytics', 'view_reports', 'export_data',
            ...
            'constituency_admin': [
                # Similar to analyst - constituency level access
                # User Management
                'view_users',
                # Data Access
                'view_dashboard', 'view_analytics', 'view_reports', 'export_data',
            ...
            'booth_admin': [
                # Similar to user - booth level access
                # Data Access
                'view_dashboard', 'create_tasks',
                # Analytics
                'view_basic_analytics',
        ...
        for role in ['superadmin', 'admin', 'manager', 'analyst', 'user', 'viewer', 'volunteer']:
            ...
            self.stdout.write(f'  {role.ljust(20)}: {perm_count} permissions')

        self.stdout.write('\nHierarchical Roles (Multi-Tenant):')
        for role in ['state_admin', 'zone_admin', 'district_admin', 'constituency_admin', 'booth_admin']:
            perm_count = RolePermission.objects.filter(role=role).count()
            self.stdout.write(f'  {role.ljust(20)}: {perm_count} permissions')
