...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        """
        Create a new user
        - If authenticated superadmin: can create admin users
        - Otherwise: register as regular user
        """
        from api.models import Organization, State

        data = request.data

        # Check if this is an authenticated request
        if request.user and request.user.is_authenticated:
            # Authenticated user creating another user (admin creating user)
            try:
                profile = request.user.profile

                # Only superadmin or state_admin can create users via this endpoint
                if not profile.is_superadmin() and not profile.is_state_admin():
                    return Response({
                        'error': 'Only superadmin or state admin can create users'
                ...
                if profile.is_superadmin():
                    # SuperAdmin creating State Admin
                    role = 'state_admin'
                    org = None
                    state = None

                    # Get organization from request or use default
                    org_name = data.get('organizationName') or data.get('organization')
                    if org_name:
                ...
                elif profile.is_state_admin():
                    # State Admin creating Zone Admin
                    role = 'zone_admin'
                    user_profile = UserProfile.objects.create(
                        user=user,
                        role=role,
            ...
            # Unauthenticated registration (regular user signup)
