...
import { EnhancedMapboxStateMap } from '../../components/maps/EnhancedMapboxStateMap';
import { WithTooltip } from '../../components/common/CardTooltip';
import {
  LineChart, Line, BarChart, Bar, PieChart, Pie, Cell, AreaChart, Area,
  XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer
} from 'recharts';
import { useStatesConfig } from '../../hooks/useStatesConfig';
...
export default function AdminStateDashboard() {
  ...
  // State-specific map data
  const [geoJsonData, setGeoJsonData] = useState<any>(null);
  const [stateConfig, setStateConfig] = useState<StateMapConfig | null>(null);

  // Dynamic states configuration from database
  const { states: statesConfig, loading: statesLoading, error: statesError, getStateByName } = useStatesConfig();

  const handleConstituencyClick = useCallback((constituency: any) => {
    console.log('Selected constituency:', constituency);
  }, []);

  // Load GeoJSON dynamically based on user's assigned state from database config
  useEffect(() => {
    const loadStateGeoJson = async () => {
      // Wait for states config to load
      if (statesLoading) {
        return;
      }

      // If states failed to load, use fallback
      if (statesError) {
        console.error('[AdminStateDashboard] Failed to load states config:', statesError);
        return;
      }

      // Get config from database via API
      let config: StateMapConfig | null = null;

      if (user?.assigned_state_name) {
        config = getStateByName(user.assigned_state_name);
      }

      // Fallback to Tamil Nadu if no state assigned or state not found
      if (!config) {
        config = statesConfig['TN'] || null;
      }

      if (!config) {
        console.warn('[AdminStateDashboard] No state configuration available');
        return;
      }

      setStateConfig(config);

      // Only load GeoJSON if it exists
      if (config.hasGeoJson) {
        try {
          // Use switch statement for static imports (required by Vite/webpack)
          // Only include imports for GeoJSON files that actually exist
          let module;
          switch (config.code) {
            case 'TN':
            ...
            case 'MH':
              ...
              break;
            case 'DL':
              module = await import('../../assets/maps/delhi-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'PB':
              module = await import('../../assets/maps/punjab-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'UP':
              module = await import('../../assets/maps/uttarpradesh-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'KA':
              module = await import('../../assets/maps/karnataka-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'GJ':
              module = await import('../../assets/maps/gujarat-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'WB':
              module = await import('../../assets/maps/westbengal-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'RJ':
              module = await import('../../assets/maps/rajasthan-constituencies.json');
              setGeoJsonData(module.default);
              break;
            case 'MP':
              ...
              break;
            default:
              console.warn(`[AdminStateDashboard] GeoJSON marked as available but not loaded for: ${config.code}`);
              setGeoJsonData(null);
          }
        } catch (error) {
          console.error(`[AdminStateDashboard] Failed to load GeoJSON for ${config.name}:`, error);
          setGeoJsonData(null);
        }
      } else {
        setGeoJsonData(null);
    ...
    loadStateGeoJson();
  }, [user?.assigned_state_name, statesLoading, statesError, statesConfig, getStateByName]);
  ...
  const loadDashboard = async (showLoader = true) => {
    try {
      ...
      // Get user's state ID for filtering
      const userStateId = user?.assigned_state;
      ...
      const [
        ...
        dashboardService.getDashboardMetrics(userStateId),
        dashboardService.getLocationSentiment(userStateId),
        dashboardService.getIssueSentiment(userStateId),
        dashboardService.getTrendingTopics(10, userStateId),
        dashboardService.getActiveAlerts(10, userStateId),
        dashboardService.getRecentSocialPosts(20, userStateId),
  ...
  useEffect(() => {
    // Load dashboard even if state_id is undefined (will show empty data)
    loadDashboard();
  ...
  const prepareSentimentData = useCallback(() => {
    ...
    // Use loaded GeoJSON if available
    const constituencies = geoJsonData ? (geoJsonData as any).features : [];
  ...
  const prepareAlertsData = useCallback(() => {
    ...
    // Use state center coordinates as base and spread markers around it
    ...
    const centerLat = stateConfig.center[1]; // latitude
    const centerLng = stateConfig.center[0]; // longitude

    return activeAlerts.slice(0, 10).map((alert, idx) => ({
      lat: centerLat + (Math.random() - 0.5) * 2, // Spread ±1 degree around center
      lng: centerLng + (Math.random() - 0.5) * 2,
      title: alert.title,
      description: alert.description,
      severity: alert.severity.toLowerCase()
    }));
  }, [activeAlerts, stateConfig]);
        ...
        <div className="bg-white/80 backdrop-blur-lg border-b border-slate-200/60 sticky top-0 z-20 shadow-sm">
          ...
          <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-slate-900">
                    {user?.organization_party_name || user?.organization_name || 'Organization'} - {user?.assigned_state_name || 'State'} Command
                  </h1>
                  <p className="text-xs text-slate-500">
                    {stateConfig?.name || user?.assigned_state_name || 'State'} • {metrics?.constituenciesCovered || stateConfig?.constituencies || 0} Constituencies • Live Data
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              {/* User Profile Widget */}
              {user && (
                <div className="hidden lg:flex items-center space-x-3 px-4 py-2 bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-lg">
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-white font-semibold text-sm shadow-md">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-medium text-indigo-700">{user.organization_party_name || user.organization_name || 'Organization'}</span>
                    <span className="text-xs text-slate-500">{user.assigned_state_name || 'State'} • {user.role || 'Role'}</span>
                  </div>
                </div>
              )}
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-2.5">
            ...
            <WithTooltip
              ...
              summary={`This metric shows the aggregated sentiment score for TVK across all constituencies in ${user?.assigned_state_name || 'the state'}. Scores above 70% indicate strong positive sentiment. The trend shows movement compared to yesterday based on social media analysis and news coverage.`}
              position="top-right"
            >
            ...
            {activeAlerts.length > 0 && (
              <WithTooltip
                title="Crisis Alerts"
                summary={`Real-time critical alerts detected across ${user?.assigned_state_name || 'the state'} based on sudden spikes in negative sentiment, high-volume issue reporting, or emergency keywords. These require immediate attention and response from the campaign team.`}
                position="top-right"
              >
          ...
          <div className="col-span-12 lg:col-span-8 space-y-3">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-slate-50 to-white px-6 py-4 border-b border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">{user?.assigned_state_name || 'State'} Sentiment Map</h2>
              ...
              <div className="relative">
                {stateConfig ? (
                  <EnhancedMapboxStateMap
                    stateName={stateConfig.name}
                    stateCode={stateConfig.code}
                    geoJsonData={geoJsonData}
                    centerCoordinates={stateConfig.center}
                    zoomLevel={stateConfig.zoom}
                    height="550px"
                  ...
                  <div className="flex items-center justify-center h-[550px] bg-slate-50 rounded-xl">
                    <div className="text-center">
                      {statesLoading ? (
                        <>
                          <RefreshCw className="w-12 h-12 text-slate-400 mx-auto mb-3 animate-spin" />
                          <p className="text-slate-600 font-medium">Loading map configuration...</p>
                        </>
                      ) : statesError ? (
                        <>
                          <AlertTriangle className="w-12 h-12 text-amber-500 mx-auto mb-3" />
                          <p className="text-slate-600 font-medium">Failed to load map configuration</p>
                          <p className="text-sm text-slate-500 mt-2">{statesError}</p>
                        </>
                      ) : (
                        <>
                          <MapPin className="w-12 h-12 text-slate-400 mx-auto mb-3" />
                          <p className="text-slate-600 font-medium">Map configuration not available</p>
                        </>
                      )}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <WithTooltip
                ...
                summary={`Top 5 topics trending on social media and news related to TVK in the last 24 hours. The growth percentage shows how fast each topic is gaining traction across ${user?.assigned_state_name || 'the state'}. Monitor these to stay ahead of the conversation.`}
                position="top-right"
              >
            ...
            >
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                <div className="px-6 py-4 border-b border-slate-100">
                  ...
                  <p className="text-sm text-slate-500 mt-1">Key concerns across {user?.assigned_state_name || 'State'}</p>
                </div>
