...
export default function ManagerDistrictDashboard() {
  ...
  // Determine scope based on user role and assignments
  const getScopeName = () => {
    switch (user?.role) {
      case 'zone_admin':
        return user?.assigned_zone_name || 'Zone';
      case 'district_admin':
        return user?.assigned_district_name || 'District';
      default:
        return user?.assigned_constituency_name || user?.constituency || 'Area';
    }
  };

  const scopeType = user?.role === 'zone_admin' ? 'Zone' : 'District';

  // District/Zone info - Read from user's assigned scope
  const [districtName, setDistrictName] = useState(getScopeName());
