 ...
 * @param stateId - Optional state ID to filter data by specific state
 */
export async function getDashboardMetrics(stateId?: string): Promise<DashboardMetrics> {
  try {
    ...
    if (stateId) {
      sentimentQuery = sentimentQuery.eq('state', stateId);  // Note: sentiment_data uses 'state' not 'state_id'
    }

    const { data: sentimentData, error: sentimentError } = await sentimentQuery.order('timestamp', { ascending: false });
    ...
    if (stateId) {
      postsQuery = postsQuery.eq('state', stateId);  // Note: social_posts uses 'state' not 'state_id'
    }
    ...
    if (stateId) {
      reportsQuery = reportsQuery.eq('state', stateId);  // Note: field_reports uses 'state' not 'state_id'
    }

    const { count: reportsCount, error: reportsError } = await reportsQuery;
    ...
    let alertsQuery = supabase
      .from('alerts')
      .select('*', { count: 'exact', head: true })
      // .eq('status', 'active')  // Commented out - status column doesn't exist in alerts table
      .in('severity', ['high', 'critical']);

    if (stateId) {
      alertsQuery = alertsQuery.eq('state', stateId);  // Note: alerts uses 'state' not 'state_id'
    }

    const { count: alertsCount, error: alertsError } = await alertsQuery;
    ...
    if (stateId) {
      issueQuery = issueQuery.eq('state', stateId);  // Note: sentiment_data uses 'state' not 'state_id'
    }
    ...
    let recentSentimentQuery = supabase
      .from('sentiment_data')
      .select('sentiment')
      .gte('timestamp', last12h);

    if (stateId) {
      recentSentimentQuery = recentSentimentQuery.eq('state', stateId);  // Note: sentiment_data uses 'state' not 'state_id'
    }

    const { data: recentSentiment } = await recentSentimentQuery;

    let previousSentimentQuery = supabase
      .from('sentiment_data')
      .select('sentiment')
    ...
    if (stateId) {
      previousSentimentQuery = previousSentimentQuery.eq('state', stateId);  // Note: sentiment_data uses 'state' not 'state_id'
    }

    const { data: previousSentiment } = await previousSentimentQuery;
    ...
    // Get unique constituencies count for this state
    let constituenciesQuery = supabase
      .from('constituencies')
      .select('id', { count: 'exact', head: true });

    if (stateId) {
      constituenciesQuery = constituenciesQuery.eq('state_id', stateId);
    }

    const { count: constituenciesCount } = await constituenciesQuery;
    ...
    return {
      ...
      topIssue,
      constituenciesCovered: constituenciesCount || 0,
      sentimentTrend,
    };
  } catch (error) {
    console.error('Error fetching dashboard metrics:', error);
    // Return generic demo data (not state-specific)
    return {
      overallSentiment: 68,
      activeConversations: 2847,
      criticalAlerts: 3,
      topIssue: 'Jobs & Employment',
 ...
 * @param stateId - Optional state ID to filter data by specific state
 */
export async function getLocationSentiment(stateId?: string): Promise<LocationSentiment[]> {
  try {
    let query = supabase
      .from('sentiment_data')
    ...
    if (stateId) {
      query = query.eq('state', stateId);  // Note: Supabase tables use 'state' not 'state_id'
    }
    ...
    // Return empty array - no hardcoded state-specific data
    return [];
  }
}
 ...
 * @param stateId - Optional state ID to filter data by specific state
 */
export async function getIssueSentiment(stateId?: string): Promise<IssueSentiment[]> {
  try {
    let query = supabase
      .from('sentiment_data')
 ...
 * @param stateId - Optional state ID to filter data by specific state
 */
export async function getTrendingTopics(limit: number = 10, stateId?: string): Promise<TrendingTopic[]> {
  try {
    let query = supabase
 ...
 * @param stateId - Optional state ID to filter data by specific state
 */
export async function getActiveAlerts(limit: number = 10, stateId?: string): Promise<ActiveAlert[]> {
  try {
    let query = supabase
      .from('alerts')
      .select('*')
 ...
 * @param stateId - Optional state ID to filter data by specific state
 */
export async function getRecentSocialPosts(limit: number = 20, stateId?: string): Promise<SocialMediaPost[]> {
  try {
    let query = supabase
      .from('social_posts')
