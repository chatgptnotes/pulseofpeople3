...
export const djangoApi = {
  ...
  /**
   * Get tenant/organization by subdomain (PUBLIC - No auth required)
   * GET /api/superadmin/tenants/by-subdomain/{subdomain}/
   *
   * Used by TenantContext to load tenant configuration on app load
   */
  async getTenantBySubdomain(subdomain: string) {
    console.log(`[DjangoAPI] 🏢 Fetching tenant by subdomain: ${subdomain}`);

    const response = await fetch(`${DJANGO_API_URL}/superadmin/tenants/by-subdomain/${subdomain}/`, {
      headers: await buildHeaders(false), // No auth required
    });

    if (!response.ok) {
      console.error(`[DjangoAPI] ❌ Failed to fetch tenant for subdomain: ${subdomain}`);
      throw new Error(`Tenant "${subdomain}" not found or inactive`);
    }

    const data = await response.json();
    console.log('[DjangoAPI] ✅ Tenant loaded successfully:', data.tenant?.name);
    return data;
  },

  /**
   * Get tenant configuration by ID (PUBLIC - No auth required)
   * GET /api/superadmin/tenants/{id}/config/
   */
  async getTenantConfig(tenantId: number) {
    const response = await fetch(`${DJANGO_API_URL}/superadmin/tenants/${tenantId}/config/`, {
  ...
  /**
   * Get list of all tenants (AUTH REQUIRED - Superadmin only)
   * GET /api/superadmin/tenants/
   */
  async getAllTenants() {
    const response = await fetch(`${DJANGO_API_URL}/superadmin/tenants/`, {
      headers: await buildHeaders(true),
    });
  ...
  /**
   * Get tenant statistics (AUTH REQUIRED - Superadmin only)
   * GET /api/superadmin/tenants/stats/
   */
  async getTenantStats() {
    const response = await fetch(`${DJANGO_API_URL}/superadmin/tenants/stats/`, {
      headers: await buildHeaders(true),
    });
  ...
  /**
   * Update tenant branding (AUTH REQUIRED - Superadmin only)
   * PUT /api/superadmin/tenants/{id}/branding/
   *
   * @param tenantId - Organization ID
   * @param brandingData - Partial update of branding, landing_page_config, theme_config, or seo_config
   */
  async updateTenantBranding(
    tenantId: number,
    brandingData: {
      branding?: Record<string, any>;
      landing_page_config?: Record<string, any>;
      theme_config?: Record<string, any>;
      seo_config?: Record<string, any>;
    }
  ) {
    console.log(`[DjangoAPI] 🎨 Updating branding for tenant ${tenantId}`);

    const response = await fetch(`${DJANGO_API_URL}/superadmin/tenants/${tenantId}/branding/`, {
      method: 'PUT',
      headers: await buildHeaders(true),
    ...
    const data = await response.json();
    console.log('[DjangoAPI] ✅ Tenant branding updated successfully');
    return data;
  },

  /**
   * Update complete tenant information (AUTH REQUIRED - Superadmin only)
   * PUT /api/superadmin/tenants/{id}/update/
   *
   * @param tenantId - Organization ID
   * @param tenantData - Any Organization fields to update
   */
  async updateTenant(tenantId: number, tenantData: Record<string, any>) {
    const response = await fetch(`${DJANGO_API_URL}/superadmin/tenants/${tenantId}/update/`, {
      method: 'PUT',
      headers: await buildHeaders(true),
