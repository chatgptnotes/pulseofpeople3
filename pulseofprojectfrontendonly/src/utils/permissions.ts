...
export type UserRole =
  | 'superadmin'
  | 'super_admin'
  // Hierarchical Multi-Party CRM Roles
  | 'state_admin'
  | 'zone_admin'
  | 'district_admin'
  | 'constituency_admin'
...
export const ROLE_HIERARCHY: Record<UserRole, number> = {
  superadmin: 1,
  super_admin: 1,
  // Hierarchical Multi-Party CRM Roles (mapped to equivalent legacy levels)
  state_admin: 2,        // Same as admin
  zone_admin: 3,         // Same as manager
  district_admin: 3,     // Same as manager
  constituency_admin: 4, // Same as analyst
  booth_admin: 5,        // Same as user
  // Legacy roles
  admin: 2,
  manager: 3,
  analyst: 4,
...
export const DEFAULT_ROLE_PERMISSIONS: Record<UserRole, PermissionName[]> = {
  ...
  // Hierarchical Multi-Party CRM Roles
  state_admin: [
    // State Admin - Same as admin (state-wide scope)
    'view_users', 'create_users', 'edit_users', 'delete_users', 'manage_roles',
    'view_dashboard', 'view_analytics', 'view_reports', 'export_data', 'import_data',
    'create_surveys', 'view_surveys', 'view_voters', 'edit_voters', 'delete_voters',
    'view_field_workers', 'manage_field_workers', 'view_field_reports', 'submit_field_reports',
  ...
  zone_admin: [
    // Zone Admin - Manager-level permissions (zone-wide scope)
  ...
  district_admin: [
    // District Admin - Manager-level permissions (district-wide scope)
    'view_users', 'create_users', 'edit_users',
    'view_dashboard', 'view_analytics', 'view_reports', 'export_data',
    'create_surveys', 'view_surveys', 'view_field_workers', 'manage_field_workers',
    'view_field_reports', 'view_alerts',
  ...
  constituency_admin: [
    // Constituency Admin - Analyst-level permissions (constituency-wide scope)
    'view_dashboard', 'view_analytics', 'view_reports', 'export_data',
    'view_surveys', 'view_field_reports', 'view_social_media',
    'view_competitor_analysis', 'view_ai_insights', 'view_alerts',
  ],
  booth_admin: [
    // Booth Admin - User-level permissions (booth-level scope)
...
export function isAdmin(userPermissions: UserPermissions | null): boolean {
  ...
  return (
    ...
    userPermissions.role === 'state_admin'
  );
}
...
export function isAdminOrManager(userPermissions: UserPermissions | null): boolean {
  ...
  return (
    ...
    userPermissions.role === 'zone_admin' ||
    userPermissions.role === 'district_admin'
  );
}
